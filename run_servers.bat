...
start "CENTENARYO Backend" cmd /k "cd backend && call venv\Scripts\activate.bat && python manage.py migrate && python manage.py runserver"

:: Give the backend a brief moment to initialize
timeout /t 2 /nobreak >nul

:: Launch Next.js Frontend in another new command window
echo [2/2] Launching Next.js Frontend Dev Server (http://localhost:3000)...
start "CENTENARYO Frontend" cmd /k "cd frontend && npm run dev"
